...
const recent3WeeksBtn = document.getElementById('recent-3-weeks-btn');
...
const TEAM_GROUPS = [
    { name: '동남아', borderColor: '#3b82f6', keywords: ['동남아1팀', '동남아2팀', '필리핀/말레이시아파트', '태국파트'] },
    { name: '중국 / 일본 / 국내', borderColor: '#6366f1', keywords: ['중국팀', '일본팀', '국내팀'] },
    { name: '유럽 / 프리미엄', borderColor: '#10b981', keywords: ['서유럽팀', '스페인/북유럽팀', '동유럽팀', '지중해/인도/아프리카팀', '프리미엄팀'] },
    { name: '미주 / 남태평양 / 지방', borderColor: '#f43f5e', keywords: ['미주팀', '남태평양팀', '부산지점', '대구지점'] }
];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (recent3WeeksBtn) {
        recent3WeeksBtn.addEventListener('click', handleRecent3WeeksSelect);
    }
});

async function handleRecent3WeeksSelect() {
    if (dashboardContent) dashboardContent.classList.remove('hidden');
    if (teamsContainer) teamsContainer.innerHTML = '';
    if (summaryContainer) summaryContainer.innerHTML = '';
    if (statusMessage) statusMessage.innerHTML = `<span style="color:#2563eb">최근 3주 데이터를 불러오는 중입니다...</span>`;
    ...
    let d = new Date();
    let cm = d.getMonth() + 1;
    let targetMonths = [cm, cm - 1 > 0 ? cm - 1 : cm - 1 + 12, cm - 2 > 0 ? cm - 2 : cm - 2 + 12];

    for (let month of targetMonths) {
        for (let w = 1; w <= 6; w++) {
            promises.push((async () => {
                const fileNameSpace = `${month}월 ${w}주차.json`;
                const fileNameNoSpace = `${month}월_${w}주차.json`;
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        return { label: `${month}월 ${w}주차`, rawData: rawData, month: month, week: w };
                    }
                } catch (e) { }
                return null;
            })());
    ...
    let validWeeks = results.filter(r => r !== null);

    validWeeks.sort((a, b) => {
        let diff = a.month - b.month;
        if (Math.abs(diff) >= 6) {
            return diff > 0 ? -1 : 1;
        }
        if (a.month !== b.month) return diff;
        return a.week - b.week;
    });

    let recentWeeks = validWeeks.slice(-3);

    if (recentWeeks.length === 0) {
        statusMessage.innerHTML = `<span style="color:red;font-weight:bold">최근 데이터를 찾을 수 없습니다.</span>`;
        if (dashboardContent) dashboardContent.classList.add('hidden');
        return;
    }

    let latestMonthNum = recentWeeks[recentWeeks.length - 1].month;
    processFetchedData(recentWeeks, latestMonthNum);
}
...
function renderTeamCharts(weeksInfo, months) {
    ...
    TEAM_GROUPS.forEach(function (group) {
        ...
        // 코드에 선언된 keywords 배열 순서대로 강제 정렬
        groupTeams.sort(function (a, b) {
            var normA = a.replace(/\s+/g, '');
            var normB = b.replace(/\s+/g, '');
            var idxA = group.keywords.findIndex(function (k) { return normA.includes(k.replace(/\s+/g, '')); });
            var idxB = group.keywords.findIndex(function (k) { return normB.includes(k.replace(/\s+/g, '')); });
            return idxA - idxB;
        });
