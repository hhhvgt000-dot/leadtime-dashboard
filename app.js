...
async function handleMonthSelect(e) {
    ...
    if (!selectedMonth) {
        ...
        if(statusMessage) statusMessage.textContent = '월을 선택해주세요.';
        return;
    }
    ...
    if (fetchedWeeks.length === 0) {
        statusMessage.innerHTML = `<span style="color:red;font-weight:bold">${selectedMonth}월에 해당하는 JSON 파일(${selectedMonth}월 1주차.json 등)을 찾을 수 없습니다.</span>`;
        if(dashboardContent) dashboardContent.classList.add('hidden');
        return;
    }

    processFetchedData(fetchedWeeks);
}

function processFetchedData(fetchedWeeks) {
    try {
        let monthMap = {};
        let weeksInfo = [];
            ...
            fw.rawData.forEach(row => {
                ...
                Object.keys(row).forEach(key => {
                    ...
                    if (k !== 'Team' && k !== '팀명' && k !== '팀' && k !== '') {
                        const translatedLabel = translateMonth(k);
...
function renderSummary(weeksInfo, months) {
    ...
    let weeksCards = weeksInfo.map((week, idx) => {
        ...
        let diff = previousTotal !== null ? t - previousTotal : null;
        previousTotal = t;
        return {
            label: week.label,
            total: t,
    ...
    function diffBadge(d) {
        var sign = d > 0 ? '+' : '';
        var color = d > 0 ? '#059669' : d < 0 ? '#e11d48' : '#94a3b8';
        var bg    = d > 0 ? '#f0fdf4' : d < 0 ? '#fff1f2' : '#f8fafc';
        return '<span style="font-size:13px;font-weight:700;color:' + color + ';background:' + bg + ';border-radius:999px;padding:2px 10px">' + sign + d.toLocaleString() + '</span>';
    }
    ...
    weeksCards.forEach(function (w) {
        ...
        html += '<span style="font-size:19px;font-weight:600;color:rgba(255,255,255,0.85);letter-spacing:0.3px">' + w.label + ' 전체 예약</span>';
        html += '<span style="font-size:36px;font-weight:800;color:#ffffff;letter-spacing:-1px">' + w.total.toLocaleString() + '</span>';
        if (w.diff !== null) {
            html += '<span style="font-size:12px;color:rgba(255,255,255,0.7)">전주 대비 &nbsp;' + diffBadge(w.diff) + '</span>';
        } else {
             html += '<span style="font-size:12px;color:rgba(255,255,255,0.0)">&nbsp;</span>'; 
        }
        html += '</div>';
    });
    html += '</div>';
...
function translateMonth(str) {
    var s = str.replace(/[^a-zA-Z0-9가-힣]/g, '');
    var map = {
        'January': 'D-0', 'Jan': 'D-0', '1월': 'D-0',
        'February': 'D-30', 'Feb': 'D-30', '2월': 'D-30',
        'March': 'D-60', 'Mar': 'D-60', '3월': 'D-60',
        'April': 'D-90', 'Apr': 'D-90', '4월': 'D-90',
        'May': 'D-120', '5월': 'D-120',
        'June': 'D-150', 'Jun': 'D-150', '6월': 'D-150',
        'July': 'D-180', 'Jul': 'D-180', '7월': 'D-180',
        'August': 'D-210', 'Aug': 'D-210', '8월': 'D-210',
        'September': 'D-240', 'Sep': 'D-240', 'Sept': 'D-240', '9월': 'D-240',
        'October': 'D-270', 'Oct': 'D-270', '10월': 'D-270',
        'November': 'D-300', 'Nov': 'D-300', '11월': 'D-300',
        'December': 'D-330', 'Dec': 'D-330', '12월': 'D-330'
    };
    function toRange(d) {
        var m = d.match(/^D-(\d+)$/);
        if (m) {
            var n = parseInt(m[1]);
            return n === 0 ? 'D-0' : 'D-' + (n - 30);
        }
        return d;
    }
    if (map[str]) return toRange(map[str]);
    if (map[s]) return toRange(map[s]);
    for (var key in map) { if (str.includes(key)) return toRange(map[key]); }
    return str;
}
